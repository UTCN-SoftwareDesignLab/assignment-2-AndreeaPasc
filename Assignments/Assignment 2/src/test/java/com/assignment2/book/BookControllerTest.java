...
public class BookControllerTest extends BaseControllerTest {
    ...
    //to fix - need performPatchWithRequestAndPath in ResultAction
    @Test
    void edit() throws Exception {
        ...
        ResultActions result = performPostWithRequestBody(BOOK + ENTITY, reqBook);
        result.andExpect(status().isOk())
                .andExpect(jsonContentToBe(reqBook));
    }
    ...
    void sell() throws Exception {
        BookDTO reqBook = BookDTO.builder()
                ...
                .build();
        //when(bookService.create(reqBook)).thenReturn(reqBook);
        when(bookService.sell(reqBook.getId(), 1)).thenReturn(true);
        ResultActions result = performPostWithRequestBodyPathVariable(BOOK + ENTITY + SELL, reqBook.getId().toString(), reqBook.getQuantity());
        result.andExpect(status().isOk());
    }
